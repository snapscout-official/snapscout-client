...
export default async function RecentlyViewed({
    ...
    const products: ProductType[] = recentlyViewedCookies
        ...
        : [];
    return (
        <div className="bg-[#F8FAFC] col-span-3 p-5 rounded-[.5rem] text-black shadow-sm space-y-2 hidden lg:flex flex-col min-h-full">
            ...
            <div className="flex-1">{children ? children : null}</div>
        </div>
    );
}
