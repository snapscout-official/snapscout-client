...
import MapRouting from "../../maps/MapRouting";
  ...
  return (
    <div>
      <div
        className={`${
          inter700.className
        } text-[#64748B] font-bold flex justify-between items-center ${
          !forDialog ? "text-xs" : "text-2xl"
        }`}
      >
        <p>{locations ? `Found ${locations.length} Results` : null}</p>
        <Button
          variant="link"
          className={`${!forDialog ? "text-xs" : "text-2xl"}`}
          asChild
          onClick={() => {}}
        >
          <a
            href={`/canvass/products?search=${search}&distance=${nextDistance}`}
          >
            Expand Radius to {nextDistance}km
          </a>
        </Button>
      </div>
      <LazyMap
        className={`w-full rounded-lg ${
          forDialog ? "h-full" : "h-[300px] mt-2"
        }`}
      >
        {locations ? (
          <>
