...
export async function getCartProducts(
    ...
    const result = await fetchWithToken({
        url: `${process.env.NEXT_PUBLIC_BACKEND_SERVICE_URL}/api/v1/agency/carts/${cart_name}`,
        method: "GET",
...
export async function getCarts(): Promise<Cart[]> {
    const result = await fetchWithToken({
        url: `${process.env.NEXT_PUBLIC_BACKEND_SERVICE_URL}/api/v1/agency/carts`,
        method: "GET",
...
export async function deleteCartProduct(product_id: string, cart_name: string) {
    const result = await fetchWithToken({
        url: `${process.env.NEXT_PUBLIC_BACKEND_SERVICE_URL}/api/v1/agency/carts/destroy-product`,
        method: "POST",
        body: JSON.stringify({
            cart_name: cart_name,
